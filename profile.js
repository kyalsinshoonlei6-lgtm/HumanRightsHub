document.addEventListener('DOMContentLoaded', () => {
  ...
  const profileCopy = {
    en: { accountTag: 'Your account', editProfile: 'Edit profile', editProfileHeading: 'Edit your profile', fullName: 'Full name', namePlaceholder: 'Enter your name', role: 'Role / title', rolePlaceholder: 'Example — Student / Member', email: 'Email address (cannot be changed)', bio: 'About you', bioPlaceholder: 'Write a little about yourself…', cancel: 'Cancel', save: 'Save changes' },
    my: { accountTag: 'သင့်အကောင့်', editProfile: 'ပရိုဖိုင် ပြင်ရန်', editProfileHeading: 'ပရိုဖိုင် ပြင်ဆင်ရန်', fullName: 'အမည်အပြည့်အစုံ', namePlaceholder: 'အမည်ရိုက်ထည့်ပါ', role: 'အခန်းကဏ္ဍ / ရာထူး', rolePlaceholder: 'ဥပမာ - ကျောင်းသား / အသင်းဝင်', email: 'အီးမေးလ်လိပ်စာ (ပြင်၍မရပါ)', bio: 'ကိုယ်ရေးအကျဉ်း', bioPlaceholder: 'သင့်အကြောင်း နည်းနည်း ရေးပါ…', cancel: 'မလုပ်တော့ပါ', save: 'ပြောင်းလဲမှုများ သိမ်းမည်' }
  };
  const applyProfileCopy = () => {
    const language = document.body.classList.contains('lang-my') ? 'my' : 'en';
    document.querySelectorAll('[data-i18n]').forEach((element) => {
      const copy = profileCopy[language][element.dataset.i18n];
      if (copy) element.textContent = copy;
    });
    document.querySelectorAll('[data-i18n-placeholder]').forEach((element) => {
      const copy = profileCopy[language][element.dataset.i18nPlaceholder];
      if (copy) element.placeholder = copy;
    });
  };
  ...
  const render = () => {
    ...
    if (elements.role) elements.role.textContent = !role || role === 'Member' || role === 'အသင်းဝင်' ? defaultRole() : role;
  ...
  const syncLanguage = () => { applyProfileCopy(); render(); };
  ...
  document.addEventListener('hrh:languagechange', syncLanguage);
